...
> 엄밀히 따지면, `this`를 선언하여 사용할 수 있지만, 그를 포함하고 있는 메소드가 호출되기 전 까지는 `undefined`상태이고, 호출 될 때 그 방법에 따라 `this`가 할당된다고 한다.
...
화살표함수의 경우 위의 두가지에 해당되지 않는 상태이며, 외부 렉시컬 스코프에서 `this`를 탐색하여 할당이 되기 때문에, `this`를 찾기 위해 상위인 `obj2`에서 탐색을 한다.

`obj2`에서는 `this`를 찾지 못하기 떄문에, 최상위 `window`까지 나오게 되고, `window`를 `this`로 참조하게 되는 것이다.

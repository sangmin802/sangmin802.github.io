...
함수 내부에서의 `this`는 다른 변수나 상수들과 다르게 렉시컬스코프와 같이 선언 시에 할당되는것이 아닌, 호출되는 방법에 의해 할당이 된다.
...
콜백함수 등 자주 접하게되는 화살표함수 또한 **함수** 이지만, 호출되는 방식에 따라 `this`가 할당되는것이 아닌, 일반 변수나 상수처럼 렉시컬스코프의 방식으로 선언될 때 `this`가 할당된다.

또한, 이 화살표함수에는 `bind`와 같은 넘겨주는 메소드가 적용되지 않는다.

렉시컬스코프의 방식으로 선언될 때 `this`가 할당되는것이 무슨의미일까?
...
화살표함수의 경우 위의 두가지에 해당되지 않는 상태이며, 선언될 때 `this`가 할당이 되기 때문에, 처음 바인딩된 `this`를 찾기 위해 `introduce`메소드 내부를 탐색하게 된다.

당연하게도, `introduce`메소드는 `this`를 알지 못하기 때문에 한단계 상위인 `obj2`에서 탐색을 한다.

마찬가지로, `this`를 찾지 못해서, 최상위 `window`까지 나오게 되고, `window`를 `this`로 참조하게 되는 것이다.

## 화살표함수와 정규함수의 this 차이

외부 깃에서 좋은 예제와 설명글으 찾을 수 있었다.
...
1. 화살표함수 내부의 `this`는 선언단계에서 형성되기 때문에, 콜백함수 내부에서 바인딩된 `this`를 찾는다.

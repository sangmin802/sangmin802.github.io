...
- 사용하고 있는 라이브러리가 어떤식으로 개발 혹은 문제를 해결하였는지 분석, 기록해보며 흥미를 느낍니다.
...
- 코드 작성도 책을 쓴다 라는 생각으로 개발을 하고자 합니다.
...
신상마켓 내부 기획전 프로모션 웹뷰를 앱 스럽게라는 마인드로 개발하였고, 분산되어 관리되고있던 광고 성과 및 사용자 행동 추적 로그 로직을 팀 내부에서 쉽게 접하고 유연하게 개발할 수 있도록 core 로직으로 분리, 관리하고 있습니다.
...
- react-query로 일괄 변경하여 데이터 요청에 대한 상태 및 캐시 데이터를 효율적으로 관리할 수 있도록 개선
- 광고 운영에 어려움을 느끼는 사용자들을 위한 기능 개발 (자동입찰 etc..)
- 페이지 단위의 기능에 대해 유지보수가 용이하고, 히스토리를 온전히 모르더라도 변경범위를 판단할 수 있는 응집도 있는 구조로 코드 리팩토링
- 서비스 특성에 맞게, 광고 상품, 소재의 상태 기준으로 단일 책임을 갖는 컴포넌트, 커스텀 훅을 사용하도록 리팩토링

...
2. 현재의 자바스크립트 환경에 쉽게 접근할 수 있고, 동적으로 생성된 값에 자유롭게 스타일링을 할 수 있음
   - 동적인 스타일을 위해 어쩔수 없이 인라인스타일을 사용해야하는 상황이 생긴다.
     > 여기서 인라인스타일이 왜 문제가 있을까라는 생각을 해보았는데, `reflow`가 발생하여 레이아웃 계산을 다시 할 때 인라인으로 작성되는 스타일은 여러 스타일이 합쳐져있는 클래스를 추가하는것으로 한번에 부여하여 일괄 계산하는것이 아니라 스타일을 하나씩 하나씩 반영하여 각각 계산하기 때문에 `reflow`의 비용이 비교적 더 크다는것 같다.
...
3. 함수 내부로 전달되는 값을 사용하여 함수 외부의 컨텍스트에 영향을 주지 않는 순수한 함수이다.
   ...
   > 여기서 React의 함수형 컴포넌트는 함수형 프로그래밍이 아니란것이 보여짐
4. 내부의 상태를 가질 수 없다.
